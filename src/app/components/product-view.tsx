  ...
  return (
    <>
      <div className="space-y-4 md:flex md:space-x-10 xl:space-x-6">
        <section className="w-full space-y-2 md:w-1/2">
          <div className="space-y-4 md:sticky md:top-10">
            <div className="w-full h-90 overflow-hidden rounded-2xl flex justify-items-center-safe relative md:shadow-lg">
              <span
                className={`absolute top-2 left-2 text-sm flex items-center-safe rounded-full border border-stone-700 px-4 z-5 ${product?.quantity && product?.quantity > 0 ? "bg-green-100/50 text-green-900" : "bg-red-100 text-red-800"}`}>
        ...
        <section className="space-y-4 md:w-1/2 md:space-y-2">
          <div>
          ...
          {/* ...follows the options */}
          {additional_options &&
            additional_options?.filter((opt) => opt.type === "options").length >
              0 &&
            additional_options
              ?.filter(
                (opt) =>
              ...
              ?.map((opt, index) => (
                <div key={index} className="w-full">
                  ...
                  <OptionInput
                    ...
                    options={
                      opt.value.split(",").map((opt) => opt.trim()) as string[]
                    }
